    ...
    public class EMessageController : Controller
    {
        ...
        public async Task<IActionResult> Index(bool showArchived = false)
        {
            ...
            IQueryable<Message> messagesQuery = _context.Messages
                .Where(m => (m.ReceiverId == employeeId || m.SenderId == employeeId) && m.ParentMessageId == null);
            ...
            if (showArchived)
            {
                return View("~/Views/Employee/Message/ArchivedMessages.cshtml", sortedMessages);
            }
            else
            {
        ...
        public async Task<IActionResult> TrashMessages()
        {
            ...
            var messages = await _context.Messages
                .Where(m => (m.ReceiverId == employeeId || m.SenderId == employeeId) && m.Status == "Trash")
                .Include(m => m.Sender)
                .Include(m => m.Receiver)
                .Include(m => m.Replies)
                .ToListAsync();
            ...
            return View("~/Views/Employee/Message/TrashMessages.cshtml", sortedMessages);
        }
